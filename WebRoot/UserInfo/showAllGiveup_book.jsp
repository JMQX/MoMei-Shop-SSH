 ...
 <table>
  <%
   ...
   int pageNow=Integer.parseInt((String)request.getAttribute("pageNow"));
	...
	if(pageNow!=1)
   {
	out.println("<a href=BookServlet?flag=show_booksGiveupByUser&userId="+user.getUserId()+"&pageNow="+(pageNow-1)+">上一页</a>");
	 ...
	 int pageCount=Integer.parseInt(s_pageCount);
			   ...
			   {
			     out.println("<a href=BookServlet?flag=show_booksGiveupByUser&userId="+user.getUserId()+"&pageNow="+i+">["+i+"]</a>");
			   }
		
			   if(pageNow!=pageCount)
			   {
			    out.println("<a href=BookServlet?flag=show_booksGiveupByUser&userId="+user.getUserId()+"&pageNow="+(pageNow+1)+">下一页</a>");
			    }
       %>
 </span>
  </div>
